...
(async () => {
  ...
  assert.match(html, /if\(AVALifecycle\.needsInstallationGateway\(\)\)return location\.replace\(AVALifecycle\.installationUrl\(\)\)/, "iPad/browser installation gateway behaviour is preserved");
  assert.match(html, /retryInitialization\.hidden=false;return/, "manual Retry remains available after both attempts fail");
